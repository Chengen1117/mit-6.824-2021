...
func (c *Coordinator) toNextPhase() {
	if c.DistPhase == int(MapPhase) {
		c.makeReduceTasks()
		c.DistPhase = int(ReducePhase)
	} else if c.DistPhase == int(ReducePhase) {
		c.DistPhase = int(AllDone)
	}
}
